...
// TestLocalTransportBashSyntax tests that bash-specific syntax works
func TestLocalTransportBashSyntax(t *testing.T) {
	transport := NewLocalTransport()
	...
	tests := []struct {
		...
		{
			name:    "bash array syntax",
			command: "arr=(one two three); echo ${arr[1]}",
			wantErr: false,
			check: func(t *testing.T, stdout string) {
				if !strings.Contains(stdout, "two") {
					t.Errorf("expected 'two' in output, got: %s", stdout)
				}
			},
		},
		{
			...
			wantErr: false,
			check: func(t *testing.T, stdout string) {
				if !strings.Contains(stdout, "test") && !strings.Contains(stdout, ".txt") {
					t.Errorf("expected 'test' without '.txt', got: %s", stdout)
				}
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if err != nil || exitCode != 0 {
					t.Errorf("unexpected error: %v (exit code: %d)", err, exitCode)
				}
				if tt.check != nil {
					tt.check(t, stdout)
				}
